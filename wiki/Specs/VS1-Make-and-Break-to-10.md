...
### Critical Review: First-Vertical-Slice Readiness
- Readiness: implementable in one engineering slice because dependencies are local-first and avoid external services.
- Critical risk today: engagement drift due to child interaction friction, not platform capability.

### Refined execution plan (epic-first with maturity gates)
- Phase 1 (must-haves): #6 App shell, #2 CPA engine/state machine, #4 concrete-to-pictorial interaction, #5 equation transfer.
- Phase 2 (support): #7 audio/accessibility and #3 telemetry/export.
- Phase 3 (pilot hardening): #8 iPhone fallback + #9 parent settings summary flow.
- Acceptance gate 1: complete Phase 1 + session persists without crash.
- Acceptance gate 2: complete Phase 2 + parent export readable in share sheet.
- Acceptance gate 3: complete Phase 3 + two smoke runs pass (iPad + iPhone simulator).
- Escalation rule: if Phase 1 miss-rates on transfer stay > 35% for first 6 sessions, reduce visual complexity before adding new content.
- Auto-check cadence: run a milestone progress sweep weekly and create/refresh one check artifact before each sprint planning.
...
- Label model:
  - Epic: `epic:vs1`
  - Tracking issue labels: `type:tracking`, `spec`, `priority:high`
  - Implementation issue labels: `type:implementation`, `feature`, `priority:*`
  - Progress issues label: `type:checks`
- Auto-progress: weekly `VS1 Milestone Progress Check` issues are generated by `.github/workflows/vs1-milestone-progress-check.yml`
